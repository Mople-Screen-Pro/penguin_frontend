    ...
    <section id="features" ref={sectionRef} className="pt-40 pb-24 px-6 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div>
          ...
          {/* Main Video */}
          <div className="animate-on-scroll scale-in max-w-5xl mx-auto">
            {/* Labels */}
            <div className="flex gap-2 mb-6 overflow-x-auto hide-scrollbar justify-center">
              {videos.map((video, i) => (
                <button
                  key={video.src}
                  onClick={() => handleVideoChange(i)}
                  className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 inline-flex items-center gap-2 ${
                    activeVideo === i
            ...
            <div className={`rounded-2xl overflow-hidden shadow-xl ring-1 ring-gray-200/60 transition-opacity duration-300 ${transitioning ? 'opacity-0' : 'opacity-100'}`}>
              {videos.map((video, i) => (
                <video
                  key={video.src}
