...
export default function Editor() {
  ...
  return (
    <section ref={sectionRef} className="py-32 px-6 bg-[#0f0f0f] border-y border-gray-800">
      <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
        ...
        <div className="order-1 lg:order-2">
          <p className="animate-on-scroll text-[120px] md:text-[160px] font-black text-gray-800 leading-none tracking-tighter select-none">Built-in</p>
          ...
          <div className="animate-on-scroll flex flex-wrap gap-3" data-delay="0.15">
            {['Auto Cursor Zoom', 'Smart Zoom Editing', 'Timeline Editor', 'Trim & Cut'].map((tag) => (
              <span key={tag} className="px-3 py-1.5 rounded-full bg-[#0a0a0a] text-sm text-gray-400 font-medium border border-gray-700">{tag}</span>
            ))}
          </div>
        </div>
      </div>
